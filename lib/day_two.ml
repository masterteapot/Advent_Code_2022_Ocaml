...
let rock = { win = Scissors; lose = Paper; draw = Rock; value = 1 }
let paper = { win = Rock; lose = Scissors; draw = Paper; value = 2 }
let scissors = { win = Paper; lose = Rock; draw = Scissors; value = 3 }
...
let assign_mixed_strategy a_string b_string =
  ...
  | "A" | "B" | "C" ->
    ...
    (match b_string with
     | "X" ->
       let s = get_counter_move o_move.win in
       s.value + 0
     | "Y" ->
       let s = get_counter_move o_move.draw in
       s.value + 3
     | "Z" ->
       let s = get_counter_move o_move.lose in
       s.value + 6
     | other -> raise (Not_a_strat other))
  | other -> raise (Not_a_strat other)
;;
...
let evalute_strategies contest =
  ...
  | a, b ->
    ...
    then b.value + 3
    else if a.strategy = Rock && b.strategy = Scissors
    then b.value
    else if a.strategy = Rock && b.strategy = Paper
    then b.value + 6
    else if a.strategy = Paper && b.strategy = Rock
    then b.value
    else if a.strategy = Paper && b.strategy = Scissors
    then b.value + 6
    else if a.strategy = Scissors && b.strategy = Paper
    then b.value
    else if a.strategy = Scissors && b.strategy = Rock
    then b.value + 6
    else 0
;;
