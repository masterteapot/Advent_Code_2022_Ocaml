...
let remove_duplicates a_string =
  let rec traverse_chars b_string result =
    ...
    | b_string ->
      ...
      let clean_char =
        if String.contains result raw_char then "" else Char.escaped raw_char
      in
...
let rec remove_empty_string a_list =
  ...
  | [] -> []
  | a_str :: tl ->
    if a_str = "" then remove_empty_string tl else a_str :: remove_empty_string tl
;;

let rec group_three in_list =
  match in_list with
  | a :: b :: c :: tl ->
    [ remove_duplicates a; remove_duplicates b; remove_duplicates c ] :: group_three tl
  | _ -> []
;;
...
let rec three_matching_char a_string b_string c_string =
  ...
  | "" -> '.'
  | a ->
    if String.contains b_string a.[0] && String.contains c_string a.[0]
    then a.[0]
    else
      three_matching_char
        (String.sub a_string 1 (String.length a_string - 1))
        b_string
        c_string
;;
...
let rec match_three_strings a_list output =
  ...
  | hd :: tl ->
    (match hd with
     | [ a; b; c ] ->
       match_three_strings tl (get_prio (three_matching_char a b c) :: output)
     | _ -> output)
;;
